  ...
  function lookupMaintainer() {
    if (!newPlace.maintainer && newPlace.addedByEmail) {
      return new Promise((resolve, reject) => {
        User.findOne({ email: newPlace.addedByEmail }, (err, user) => {
...
Schema.statics.update = function update(id, data, cb = () => {}) {
  const Place = this;
...
Schema.statics.getById = function getById(id, callback) {
  const Place = this;

  Place.findOne({ _id: mongoose.Types.ObjectId(id) }).populate('maintainer', 'name').exec(callback);
};

Schema.statics.markAsConfirmed = function markAsConfirmed(id, callback) {
  const Place = this;
...
Schema.statics.addReview = function addReview(id, data, callback) {
  const Place = this;
...
Schema.statics.addEvent = function addEvent(id, data, callback) {
  const Place = this;
...
Schema.statics.editEvent = function editEvent(id, data, callback) {
  const Place = this;
...
Schema.statics.addPromotion = function addPromotion(id, data, callback) {
  const Place = this;
...
Schema.statics.addJob = function addJob(id, data, cb = () => {}) {
  const Place = this;
...
Schema.statics.getCountries = function getCountries(callback) {
  const Place = this;

  Place.distinct('address.country').exec(callback);
};

Schema.statics.getLocalities = function getLocalities(country, callback) {
  const Place = this;
  const regex = new RegExp(`^${country.replace(/-/g, ' ').toLowerCase()}`, 'i');

  Place.find({ 'address.country': { $regex: regex } }).distinct('address.locality').exec(callback);
};
...
function preprocessFields(place, cb) {
  ...
  ReligionGroup.find(religionGroup, (err, religionGroups) => {
    if (err) throw err;
  ...
  Denomination.addIfNotExists(place.denominations, place.religion);
